...
位置情報メッセージ／デジタル宝探しのジャンル。少人数で使う **PoC**（仮本番運用・複数端末利用を見据える）。
...
少人数で使う PoC。**認証は Google OAuth でスコープ内**（投稿の所有＝編集/削除を本人に限るため。Phase 7。経緯は #29-31）。
...
- **過剰実装しない**。少人数の PoC。§2 の Non-Goals を勝手に作らない。
...
## 16. デザイン方針（UI トーン / 原研哉風ミニマル）

UI は **「原研哉風ミニマル」** を恒久トーンとする（#16 で確立）。**余白・静けさ・最小限の要素・抑えた色と装飾。** 見た目は ui-designer が own（§14）だが、ロジック側（`main.js`）が持つ視覚値もこの方針に揃える。

- **配色は無彩色のモノトーン基調**: UI（モーダル等）は墨 `#1c1b18` × 生成りの紙 `#fbfaf7`。彩度は最小限、装飾（角丸・影・枠）は淡く細く。
- **アクセントは2色だけ・役割固定**:
  - **ピン＝朱 `#bf5a3c`**（落款/印泥の色）。
  - **現在地＝青 `#1a73e8`**（`circleMarker`）。
  - **暖色↔寒色で対比**させて識別する。**この2色の役割（朱＝ピン／青＝現在地）は壊さない。** 色を増やすとき（例: 将来 #31/#32 で「自分／他人のピン」を分ける）は、**朱を基準色に2色目を足す**。
- **ベースマップは 70% グレー化**（`.leaflet-tile-pane` にだけ `filter: grayscale(.7)`）。色の主役をピンと現在地に譲る。マーカーは別ペインなので色を保つ。
- **絵文字を多用しない**。状態の印は CSS の細い線/幾何で静かに描く。
- **文言は簡潔で静かなトーン**（手紙の語り）。例: 送り主は `[name]より`、ロックは「拒絶」でなく「まだ・ここに在る」の待ちの言葉。

> 値の最終調整（しきい値・ピン色味など）は現地確認（#18）前提の暫定。トーンの原則そのものは恒久ルール。
